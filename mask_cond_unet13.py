...
class Unet(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.final_res_block_aux = block_klass(dim * 2, dim, time_emb_dim=time_dim)
        self.final_conv_aux = nn.Conv2d(dim, self.out_dim * out_mul, 1)
    ...
    def forward(self, x, time, mask, x_self_cond = None, **kwargs):
        ...
        aux_x1 = self.final_res_block_aux(aux_fea, t)
        aux_x1 = F.interpolate(self.final_conv_aux(aux_x1), size=x1.shape[-2:], mode='bilinear')
        return x1, x2, aux_x1
