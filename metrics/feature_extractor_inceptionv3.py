...
from metrics.interpolate_compat_tensorflow import interpolate_bilinear_2d_like_tensorflow1x
...
class FeatureExtractorInceptionV3_new(FeatureExtractorInceptionV3):
    INPUT_IMAGE_SIZE = 299
    ...
    def forward(self, x):
        ...
        # x = x.to(torch.float64)  # fix batch-dependent performance bug
        # N x 3 x ? x ?
        ...
        # x = (x - 128) * torch.tensor(0.0078125, dtype=torch.float32, device=x.device)  # really happening in graph
        x = (x - 0.5) / 0.5  # but this gives bit-exact output _of this step_ too
        # x = 2*x-1
        # N x 3 x 299 x 299
