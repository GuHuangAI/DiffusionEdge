...
from denoising_diffusion_pytorch.ldm import LatentDiffusion
from denoising_diffusion_pytorch.data import ImageMaskDataset, ImageDataset
...
def main(args):
    ...
    dataset = ImageDataset(
        ...
        # mask_folder=data_cfg.mask_folder,
        image_size=model_cfg.image_size,
...
class Trainer(object):
    def __init__(
        ...
        self.opt = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()),
                                  lr=train_lr)
        lr_lambda = lambda iter: (1 - iter / train_num_steps * 0.9837216) ** 0.95
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            if isinstance(self.model, nn.parallel.DistributedDataParallel):
                                all_images = self.model.module.sample(batch['cond'])
                            elif isinstance(self.model, nn.Module):
                                all_images = self.model.sample(batch['cond'])
