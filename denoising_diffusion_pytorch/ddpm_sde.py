...
class DDPM(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        assert not torch.isnan(self.sqrt_recip_alphas_cumprod).all()
        register_buffer('sqrt_recipm1_alphas_cumprod', torch.sqrt(1. / alphas_cumprod - 1))
        assert not torch.isnan(self.sqrt_recipm1_alphas_cumprod).all()
        ...
        assert not torch.isnan(self.posterior_mean_coef1).all()
        register_buffer('posterior_mean_coef2', (1. - alphas_cumprod_prev) * torch.sqrt(alphas) / (1. - alphas_cumprod))
        assert not torch.isnan(self.posterior_mean_coef2).all()
    ...
    def model_predictions(self, x, t, cond, x_self_cond = None, clip_x_start = False, rederive_pred_noise = False):
        model_output = self.model(x, t, cond)[1]
...
class LatentDiffusion(DDPM):
    ...
    def on_train_batch_start(self, batch):
        ...
        if self.scale_by_std and (not self.scale_by_softsign):
            ...
            print(f'#3## USING SOFTSIGN SCALE !')
    ...
    def p_losses(self, x_start, t, *args, **kwargs):
        ...
        # loss_simple += self.get_loss(noise_pred, target2, mean=False).mean([1, 2, 3])
        # print(noise_pred.mean())
        loss_simple += torch.abs(noise_pred - target2).mean([1, 2, 3])
    ...
    @torch.no_grad()
    def sample_fn_ddim(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False):
        batch, device, total_timesteps, sampling_timesteps, eta, objective = shape[0], \
            self.betas.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            time_cond = torch.full((batch,), time, device=device, dtype=torch.long)
            self_cond = x_start if self.self_condition else None
            pred_noise, x_start, *_ = self.model_predictions(img, time_cond, cond, clip_x_start=False,
    ...
    def sample_fn(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False):
        ...
        for t in reversed(range(self.num_timesteps-self.sampling_timesteps, self.num_timesteps)):
    ...
    @torch.no_grad()
    def sample_debug(self, batch_size=16, up_scale=1, cond=None, mask=None, denoise=True):
        # image_size, channels = self.image_size, self.channels
        channels = self.channels
        image_size = cond.shape[-2:]
        if cond is not None:
        ...
        down_ratio = self.first_stage_model.down_ratio
        if self.cfg.model_name == 'cond_unet8' or self.cfg.model_name == 'cond_unet13':
            z, aux_out = self.sample_fn_debug(
                (batch_size, channels, image_size[0] // down_ratio, image_size[1] // down_ratio),
                up_scale=up_scale, unnormalize=False, cond=cond, denoise=denoise)
        else:
            zs = self.sample_fn_debug((batch_size, channels, image_size[0] // down_ratio, image_size[1] // down_ratio),
                               up_scale=up_scale, unnormalize=False, cond=cond, denoise=denoise)
            aux_out = None

        if self.scale_by_std:
            for i in range(len(zs)):
                zs[i] = 1. / self.scale_factor * zs[i].detach()
            if self.cfg.model_name == 'cond_unet13':
                aux_out = 1. / self.scale_factor * aux_out.detach()
        elif self.scale_by_softsign:
            z = z / (1 - z.abs())
        ...
        x_recs = []
        for i in range(len(zs)):
            x_rec = self.first_stage_model.decode(zs[i])
            x_rec = unnormalize_to_zero_to_one(x_rec)
            x_rec = torch.clamp(x_rec, min=0., max=1.)
            x_recs.append(x_rec)
        if self.cfg.model_name == 'cond_unet13':
        ...
        if aux_out is not None:
            return x_rec, aux_out
        return torch.cat(x_recs, dim=0)

    @torch.no_grad()
    def sample_fn_debug(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False):
        batch, device, total_timesteps, sampling_timesteps, eta, objective = shape[0], \
                                                                             self.betas.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective
