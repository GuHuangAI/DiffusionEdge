...
### Compared to unet7:
# 1. add aux branch for side output
...
class Unet(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if cfg.cond_net == 'effnet':
            ...
            fuseplanes = [f_condnet * s for s in [1, 4]]
        ...
        # fuseplanes =
        for i in range(2):
            self.dilations.append(CDCM(fuseplanes[i], 64))
            self.attentions.append(CSAM(64))
            self.conv_reduces.append(MapReduce(64))
        # elif self.sa:
        #     self.attentions = nn.ModuleList()
        #     for i in range(2):
    ...
    def forward(self, x, time, mask, x_self_cond = None, **kwargs):
        ...
        ### aux outputs
        e1 = self.conv_reduces[0](self.attentions[0](self.dilations[0](hm[0])))
        e1 = F.interpolate(e1, mask.shape[-2:], mode="bilinear", align_corners=False)

        e2 = self.conv_reduces[1](self.attentions[1](self.dilations[1](hm[2])))
        e2 = F.interpolate(e2, mask.shape[-2:], mode="bilinear", align_corners=False)
        ...
        return x1, x2, (torch.sigmoid(e1), torch.sigmoid(e2))
