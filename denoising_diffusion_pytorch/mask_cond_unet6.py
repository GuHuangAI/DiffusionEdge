...
class Unet(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.init_conv = nn.Sequential(
            nn.Conv2d(input_channels + init_dim, init_dim, 7, padding = 3),
    ...
    def forward(self, x, time, mask, x_self_cond = None):
        ...
        m = self.init_conv_mask(mask)
        if self.cond_pe:
            m = self.cond_pos_embedding(m)
        x = self.init_conv(torch.cat([x,
                                      F.interpolate(m, size=x.shape[-2:], mode="bilinear")], dim=1))
        r = x.clone()
        ...
        hm = []
        hm2 = []
        for (block1, block2, attn, downsample), (m_block, m_attn, m_downsample), relation_layer \
                in zip(self.downs, self.downs_mask, self.relation_layers_down):
            x = block1(x, t)
            h.append(x)
            h2.append(x.clone())
            m = m_block(m, t)
        ...
        x = x + F.interpolate(m, size=x.shape[2:], mode="bilinear", align_corners=True)
