...
class DDPM(nn.Module):
    ...
    #     return x_noisy
    def q_sample(self, x_start, noise, t, K1, K2, C):
        time = t.reshape(C.shape[0], *((1,) * (len(C.shape) - 1)))
        x_noisy = x_start + K1 / 3 * time**3 + K2 / 2 * time**2 + C * time + torch.sqrt(time) * noise
        return x_noisy
    ...
    def pred_x0_from_xt(self, xt, noise, t, K1, K2, C):
        time = t.reshape(C.shape[0], *((1,) * (len(C.shape) - 1)))
        x0 = xt - K1 / 3 * time**3 - K2 / 2 * time**2 - C * time - torch.sqrt(time) * noise
        return x0

    def pred_xtms_from_xt(self, xt, noise, K1, K2, C, t, s):
        # noise = noise / noise.std(dim=[1, 2, 3]).reshape(C.shape[0], *((1,) * (len(C.shape) - 1)))
        time = t.reshape(C.shape[0], *((1,) * (len(C.shape) - 1)))
        s = s.reshape(C.shape[0], *((1,) * (len(C.shape) - 1)))
        mean = xt + K1/3*(-3 * s * time**2 + 3 * time * s**2 - s**3) \
               + K2/2*(-2 * time * s + s**2) - C * s - s / torch.sqrt(time) * noise
    ...
    def p_losses(self, x_start, t, *args, **kwargs):
        ...
        K1 = 1. * torch.ones_like(x_start)
        K2 = -1. * torch.ones_like(x_start)
        # C = noise - x_start  # t = 1000 / 1000
        C = -1 * x_start - K1 / 3 - K2 / 2          # U(t) = K1/3 * t**3 + K2/2 * t**2 + Ct, U(1) = -x0
        x_noisy = self.q_sample(x_start=x_start, noise=noise, t=t, K1=K1, K2=K2, C=C)
        ...
        x_rec = self.pred_x0_from_xt(x_noisy, noise_pred, t=t, K1=K1, K2=K2, C=C_pred)
        loss_dict = {}
        prefix = 'train'
    ...
    def sample(self, batch_size=16, up_scale=1, cond=None, denoise=True):
        ...
        return self.sample_fn((batch_size, channels, image_size[0], image_size[1]), up_scale=up_scale,
                              unnormalize=True, cond=cond, denoise=denoise)
    ...
    def sample_fn(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False):
        ...
        for i, time_step in enumerate(time_steps):
            ...
            C, noise = pred[:2]
            # Correct C
            x0 = self.pred_x0_from_xt(img, noise, cur_time, K1, K2, C)
            if self.clip_x_start:
                x0.clamp_(-1., 1.)
                # C.clamp_(-5/6, 7/6)
                # C.clamp_(-2., 2.)
            C = -1 * x0 - K1 / 3 - K2 / 2
            img = self.pred_xtms_from_xt(img, noise, K1, K2, C, cur_time, s)
