...
from denoising_diffusion_pytorch.wcc import WCC
### Compared to unet4:
# 1. add all wave-Conv.
...
class BlockFFT(nn.Module):
    def __init__(self, dim, h, w, groups = 8):
        ...
        self.wcc = WCC(in_channels=dim, out_channels=dim)
...
class Unet(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        block_klass = partial(ResnetBlockFFT, groups = resnet_block_groups)
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                block_klass(dim_in, dim_in, feature_size_list[ind][0], feature_size_list[ind][1], time_emb_dim = time_dim),
                block_klass(dim_in, dim_in, feature_size_list[ind][0], feature_size_list[ind][1], time_emb_dim = time_dim),
        ...
        mid_dim = dims[-1]
        self.mid_block1 = block_klass(mid_dim, mid_dim, feature_size_list[-1][0], feature_size_list[-1][1], time_emb_dim = time_dim)
        self.mid_attn = Residual(PreNorm(mid_dim, Attention(mid_dim)))
        self.mid_block2 = block_klass(mid_dim, mid_dim, feature_size_list[-1][0], feature_size_list[-1][1], time_emb_dim = time_dim)
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.ups.append(nn.ModuleList([
                block_klass(dim_out + dim_in, dim_out, feature_size_list[len(dim_mults)-ind-1][0], feature_size_list[len(dim_mults)-ind-1][1], time_emb_dim = time_dim),
                block_klass(dim_out + dim_in, dim_out, feature_size_list[len(dim_mults)-ind-1][0], feature_size_list[len(dim_mults)-ind-1][1], time_emb_dim = time_dim),
        ...
        self.final_res_block = block_klass(dim * 2, dim, feature_size_list[0][0], feature_size_list[0][1], time_emb_dim = time_dim)
        self.final_conv = nn.Conv2d(dim, self.out_dim * out_mul, 1)

        self.final_res_block2 = block_klass(dim * 2, dim, feature_size_list[0][0], feature_size_list[0][1], time_emb_dim=time_dim)
